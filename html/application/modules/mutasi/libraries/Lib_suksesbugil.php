...
class Lib_suksesbugil {
	...
	function __construct() {
		...
		if (isset($this->base_suksesbugil['cookie_path'])) {
			$this->cookies_path = $this->base_suksesbugil['cookie_path'];
		}
		$this->set_headers();
		...
		// Set Curl Init
		$this->cookie_filename = (isset($this->base_mutasi['cookies_filename']) ? $this->base_mutasi['cookies_filename'] : 'cookies.txt');
	...
	function login($login_params = null) {
		if (!isset($login_params)) {
			$login_params = $this->login_params;
		}
		if (!isset($login_params['username'])) {
			exit("Login params not yet defined!");
		}
		// Set Curl Initialize for multiple cookies
		$this->set_curl_init($this->create_curl_headers($this->headers), $login_params['username']);
		// LOAD FRESH CONFIG
		$this->base_suksesbugil = $this->CI->config->item('base_suksesbugil');
		// REPLACE WITH NEW LOGIN
		if (isset($this->base_suksesbugil['url']['authcode']) && isset($login_params['username'])) {
			$this->base_suksesbugil['url']['authcode'] = str_replace('#[USERNAME]#', $login_params['username'], $this->base_suksesbugil['url']['authcode']);
		}
		if (isset($this->base_suksesbugil['params']['login']['entered_login']) && isset($login_params['username'])) {
			$this->base_suksesbugil['params']['login']['entered_login'] = str_replace('#[USERNAME]#', $login_params['username'], $this->base_suksesbugil['params']['login']['entered_login']);
		}
		if (isset($this->base_suksesbugil['params']['login']['vb_login_md5password']) && isset($login_params['password'])) {
			$this->base_suksesbugil['params']['login']['vb_login_md5password'] = str_replace('#[PASSWORD]#', md5($login_params['password']), $this->base_suksesbugil['params']['login']['vb_login_md5password']);
		}
		if (isset($this->base_suksesbugil['params']['login']['vb_login_md5password_utf']) && isset($login_params['password'])) {
			$this->base_suksesbugil['params']['login']['vb_login_md5password_utf'] = str_replace('#[PASSWORD]#', md5($login_params['password']), $this->base_suksesbugil['params']['login']['vb_login_md5password_utf']);
		}
		if (isset($this->base_suksesbugil['params']['pin']['pin']) && isset($login_params['pin'])) {
			$this->base_suksesbugil['params']['pin']['pin'] = str_replace('#[PIN]#', $login_params['pin'], $this->base_suksesbugil['params']['pin']['pin']);
		}
	...
	function get_data_transaction_by($by_type, $by_value) {
		...
		$value = ((is_string($value) || is_numeric($value)) ? $value : 'all');
		$value = strtolower($value);
		$login_params = $this->get_login_params('bank', $value);
		switch (strtolower($by_type)) {
			case 'bank':
			default:
				if (isset($this->base_suksesbugil['get_data_url'][$value])) {
					$this->set_curl_init($this->create_curl_headers($this->headers), 'bank');
					$this->login();
	...
	function set_auto_approve($bank_code, $input_params) {
		...
		$this->set_curl_init($this->create_curl_headers($this->headers), $value);
		$this->login();
		curl_setopt($this->ch, CURLOPT_URL, $this->base_suksesbugil['get_data_url'][$value]);
		curl_setopt($this->ch, CURLOPT_REFERER, $this->base_suksesbugil['url']['menu']);
		$this->curlexec();
