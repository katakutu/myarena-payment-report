	...
	<div class="page-content">
			...
			<div class="col-md-12">
				...
				<ul class="page-breadcrumb breadcrumb">
					...
					<li>
						...
						<a href="<?= base_url($base_path . '/report/viewdeposit/' . $collect['bank_type_data']->bank_code);?>"><?=$collect['bank_type_data']->bank_name;?></a>
						<i class="fa fa-angle-right"></i>
					</li>
					<li>
		...
		<div class="row">
									...
									if (is_array($collect['bank_type']) && count($collect['bank_type'])) {
										foreach ($collect['bank_type'] as $val) {
											...
											<li class="<?=$li_href_active;?>">
												<a href="<?= base_url($base_path . '/report/viewdeposit/' . $val->bank_code);?>">
													<i class="fa fa-university"></i>
			...
			<div class="col-md-6 col-xs-12 pull-right">
				<!-- BEGIN FORM-->
				<form action="<?= base_url($base_path . '/report/viewdeposit/' . $collect['bank_type_data']->bank_code);?>" class="form-horizontal" role="form" id="searh-form" method="post">
				...
				if (is_array($collect['bank_type']) && count($collect['bank_type'])) {
					foreach ($collect['bank_type'] as $val) {
						...
						<div class="col-md-2 col-xs-4">
							<a class="<?=$div_href_active;?>" href="<?= base_url($base_path . '/report/viewdeposit/' . $val->bank_code);?>">
								<i class="fa fa-university"></i>
								<span><?=$val->bank_name;?></span>
							</a>
						</div>
										...
										<tbody>
											...
											foreach($collect['bank_accounts']['data'] as $keval) {
												...
												<tr>
													...
													<td>
														<?php
														if (isset($keval->transaction_all->count_unit)) {
															if ((int)$keval->transaction_all->count_unit > 0) {
																echo number_format($keval->transaction_all->count_unit);
															} else {
																echo "-";
															}
														} else {
													...
													<td>
														<?php
														if (isset($keval->transaction_all->sum_amount)) {
															if ((int)$keval->transaction_all->sum_amount > 0) {
																echo number_format($keval->transaction_all->sum_amount, 2);
															} else {
																echo "-";
															}
														} else {
													...
													<td class="text-center">
														<a class="btn btn-sm btn-primary" href="<?php echo base_url("{$base_path}/report/viewdeposittransaction/{$keval->seq}"); ?>">
